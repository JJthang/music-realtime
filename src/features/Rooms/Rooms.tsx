...
const Rooms: React.FC<RoomsProps> = () => {
  ...
  const onsearchRoom = debounce(async (val: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
      const inputValue = val.target.value;
      if (inputValue) {
        return await searchRooms(orderByChild("nameRoom"), startAfter(inputValue));
      }
      searchRooms()
    },
    500
